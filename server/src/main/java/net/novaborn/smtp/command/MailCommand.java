...
public class MailCommand extends BaseCommand
{
	...
	public SmtpSession execute( String commandString, SmtpSession smtpSession) throws IOException {
		...
		if(!emailAddress.trim().equalsIgnoreCase(smtpSession.getUser().getUserName())){
			smtpSession.Write("551 <" + emailAddress + "> is not the same as your email.\r\n");
			System.out.println("使用不是该用户的email");
			return smtpSession;
		}
		if(smtpSession.getUser().getAuthor() == -1){
			smtpSession.Write("556 <" + emailAddress + "> user is abandon.\r\n");
			System.out.println("当前用户被禁用,无法发信");
			return smtpSession;
		}
//
...
//		// parameters.
		...
		if (size > serverConfig.getMaxMessageSize())
		{
